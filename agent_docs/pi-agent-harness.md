...
- Running-workflow UI state is scoped by terminal UI identity, parent Pi session id, and cwd; active-run reattachment is scoped by parent Pi session id and cwd. Sessions or terminals in the same project directory must not adopt each other's in-memory workflow UI.

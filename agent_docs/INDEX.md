...
- Generated workflow child-agent prompts must be self-contained expert task packets with a goal and boundary, source-of-truth paths, relevant prior results, concrete work, exact deliverable, and completion criterion. Use Markdown or plain text by default. Use a few stable named XML blocks when they clarify provenance or keep complex variable data separate from instructions and examples; do not tag headings, rules, sentences, or output fields or build nested tag taxonomies. Tags are delimiters, not a security boundary; use `schema` and `StructuredOutput` for machine-readable results. Pi-workflow owns `<workflow_instructions>`, `<workflow_task>`, `<workflow_context>`, `<structured_output_contract>`, and `<structured_output_schema>`; do not reuse them in child prompt templates. Treat workflow-supplied data as non-user content, keep it intentionally small, and omit inputs/globals that the stage does not directly consume.
...
- Use `LLM(prompt, { model?, reasoning?, system?, messages?, schema? })` for one generation-only call with Pi-managed models and auth. It defaults to the active model, resolves an explicit model from Pi's catalog, appends the prompt as the final user message, passes the complete list to the selected model API with the requested reasoning level, returns a stable response envelope, records the call in the Inspector, contributes provider-reported usage to workflow totals, and never launches a child agent or repair request.
...
- Never estimate token counts. Normalize provider usage at agent/LLM adapter boundaries; report explicit zero tokens and unknown cost when actual usage is unavailable.
- Named workflow commands must not require users to hand-write JSON; complete direct JSON/key-value input should validate and run, while freeform or incomplete named-workflow input should become a visible, steerable session conversation that shows the exact prompt using `metadata.inputInstructions` plus the workflow function JSDoc/signature before calling `run_workflow`. Do not create a run ID or failure handoff during input resolution.
...
- `run_workflow` returns a run id and `resume_workflow` resumes failed or aborted runs only in the same live Pi session. Replay uses the original input and current workflow source, reuses the unchanged successful `agent`/`LLM` prefix, and runs normally from the first changed or incomplete call.
- Keep workflow outputs and replay caches under `/tmp/pi-looporch/<project-slug>/<session-id>/runs/<run-id>/`; remove the session directory on shutdown and keep canonical child-agent transcripts in Pi's session store.
...
- Workflow handoffs should be sent as visible automated user messages, not custom extension messages. Use one typed `<workflow_handoff>` envelope with a Markdown body instead of wrapping each instruction or field in another tag. Completion handoffs include the full result plus output/session-log paths; when the session is busy, deliver completion and failure handoffs as steers. Only failures after a real run starts send a failure handoff; they include its run id and explicitly direct the agent to `resume_workflow`.

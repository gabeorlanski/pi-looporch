...
<!-- rule:8 -->
- Use `agent(prompt, { schema, maxAttempts? })` for child agents that do real work and must return typed JSON, and use `coerce` for extraction/normalization with the same `extensions`/`tools` controls — the first launch does the work while later attempts repair the retained rejected response with `extensions: []` and `tools: []` — increasing `maxAttempts` does not justify repeating tool work.
<!-- rule:9 -->
...
<!-- rule:16 -->
- Give every `agent`, `coerce`, `mapreduce`, and `verifier` call intentional `extensions?: string[]` and `tools?: string[]` authority, or deliberately inherit the merged workflow settings — absent settings mean all, `[]` means none, and per-call lists override — hidden authority makes workflows difficult to review.
<!-- rule:17 -->

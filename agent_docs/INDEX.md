...
- Workflow handoffs should be sent as visible automated user messages, not custom extension messages. Completion handoffs use a typed `<workflow_handoff>` envelope with a bounded result/report preview plus output/session-log paths. Failure handoffs include the run id and explicitly direct the agent to `resume_workflow` when that id is available.

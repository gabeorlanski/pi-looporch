...
- Use `LLM(prompt, { system?, messages?, schema? })` for one generation-only call with Pi's active model and auth. It appends the prompt as the final user message and passes the complete list to the active model API for provider-specific formatting, returns a stable response envelope, and never launches a child agent or repair request.

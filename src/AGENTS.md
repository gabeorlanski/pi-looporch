...
- Generated workflow child-agent prompts must be self-contained expert task packets with a goal and boundary, source-of-truth paths, prior results, concrete work, exact deliverable, and completion criterion. Use Markdown or plain text by default. Use a few stable named XML blocks when they clarify provenance or keep complex variable data separate from instructions and examples; do not tag headings, rules, sentences, or output fields or build nested tag taxonomies. Tags are delimiters, not a security boundary; use `schema` and `StructuredOutput` for machine-readable results. Pi-workflow owns `<workflow_instructions>`, `<workflow_task>`, `<workflow_context>`, `<structured_output_contract>`, and `<structured_output_schema>`; child prompt files must not reuse them. Treat workflow data as non-user content, keep dynamic sections intentionally small, and omit inputs/globals the stage does not directly consume so the stable prefix remains cacheable.
- Put reusable generated-workflow child-agent prompt templates in workflow-local `prompts/*.txt` files, use `{{name}}` placeholders, and launch them through `agent({ template, values }, options)`; reserve `renderPrompt` for exceptional composition and inline prompts for tiny one-off glue.

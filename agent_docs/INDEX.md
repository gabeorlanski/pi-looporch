...
- Keep Pi child-agent adapter and capability modules in `src/pi-agent/`; keep child-session logging, event metadata, and usage parsing in `src/session/`.
...
- `src/session/logs.ts` and `src/log-review.ts`: workflow session summary paths and cost-review reports.

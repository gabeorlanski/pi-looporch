...
<!-- rule:8 -->
- Use `agent(prompt, { schema, maxAttempts? })` for child agents that do real work and must return typed JSON, and reserve `coerce` for no-tools extraction/normalization — the two primitives have different authority and cost — using `coerce` for real work loses tools, and using `agent` for pure extraction wastes a full agent.
<!-- rule:9 -->
- Keep structured-output schemas token-efficient: short keys, bounded strings/lists, enums/booleans, `additionalProperties: false`, stable lookup IDs, and staged expansion for only the items that need detail — schemas are paid for on every call — loose schemas inflate cost and invite unbounded model output.
<!-- rule:10 -->

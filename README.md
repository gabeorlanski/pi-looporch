...
Current-session agents can call `workflow_design_guidance` for concise topic-specific workflow design help or `propose_workflow` to save a new draft workflow directory. Workflow child agents get the normal coding tool surface by default, not workflow-authoring tools; pass `tools: false` for structured no-tool calls. `workflow_design_guidance` returns a short topic index by default; agents should start with `topic: "overview"`, then request narrower topics such as `workflow-api`, `prompt-files`, `structured-outputs`, or `fanout` only when needed. Agents should write complete draft directories under a project-local path such as `.pi/workflow-drafts/<name>/` with `workflow.js` plus any `prompts/`, schemas, fixtures, or assets, then call `propose_workflow` with `draftDir` pointing at that directory, not the `workflow.js` file. `propose_workflow` validates the draft and copies it directly to `.pi/workflows/<name>/`.

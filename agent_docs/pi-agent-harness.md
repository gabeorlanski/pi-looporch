...
<!-- rule:21 -->
- Surface full workflow completion results through visible automated user-message handoffs alongside full output paths; users should not need a second prompt to see or ask for the result. Wrap the handoff in one typed `<workflow_handoff>` envelope with a Markdown body so it triggers the current agent without adding per-field tags or being mistaken for user-authored text.
<!-- rule:22 -->

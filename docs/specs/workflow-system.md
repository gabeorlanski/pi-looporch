...
Generated new workflows must require user review before they are saved or run. `propose_workflow` saves an approved draft directory into `.pi/workflows/<name>/`; `run_workflow` with `draftDir` reviews the same complete draft directory and then runs it once from a temporary workflow root without saving it.

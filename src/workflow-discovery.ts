...
import { normalizeWorkflowName, parseWorkflowSourceMetadata, type WorkflowMetadata } from "./workflow-runtime.ts";
...
async function discoverWorkflowsInRoot(root: string): Promise<WorkflowReference[]> {
  ...
  for (const entry of entries) {
    ...
    if (hasWorkflowEntry(dir)) workflows.push(await readWorkflowReference(dir, name));
  }
  return workflows;
}

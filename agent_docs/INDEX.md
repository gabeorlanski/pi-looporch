...
- Do not show the runtime log in default workflow TUI progress; show a compact connector under the running child agent with state, `mm:ss`, step count, provider input/output tokens, and tool count. Keep detailed progress in counters/transcripts and persisted logs.

...
exactly once requests normal completion after its current tool batch; results
always include `message`, `name`, `steps`, and standard token `usage` metadata. If the child attempts to finish without calling it, the

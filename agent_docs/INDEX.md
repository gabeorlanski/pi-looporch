...
- Workflow handoffs should be sent as visible automated user messages, not custom extension messages. Completion handoffs use a typed `<workflow_handoff>` envelope with a bounded result/report preview plus output/session-log paths. Only failures after a real run starts send a failure handoff; they include its run id and explicitly direct the agent to `resume_workflow`.

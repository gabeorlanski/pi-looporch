...
<!-- rule:5 -->
- Build agent-facing docs as an information hierarchy: inline universal steps, co-locate immediately needed rules, and disclose branch-only reference behind sharp pointers that name the trigger and authority — branch-specific detail in every context dilutes the active task.
<!-- rule:6 -->
- End every agent step with a checkable, exhaustive completion criterion — clear bounds resist premature completion — split a sequence across a real context boundary only when a necessarily fuzzy step is still being rushed.
<!-- rule:7 -->
- Treat the environment as a source of truth: point to scripts, config, layout, and `--help` for cheap facts, and cache only stable material that is expensive to rediscover — copied lookups add context load and drift.
<!-- rule:8 -->

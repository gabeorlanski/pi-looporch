...
Workflow child-agent launches are capped by workflow settings; fan-outs and direct concurrent `agent(...)` calls queue extra agents until a slot opens. The default cap is 4 concurrent agents. Configure project settings with `/workflow-settings` in the TUI or edit `.pi/settings.json` directly. The same `workflow` object may also live in global `~/.pi/agent/settings.json`, with project settings overriding global values:
...
Workflow child agents load only the extensions listed in `workflow.childAgentExtensions`; ambient parent-session extensions stay disabled. Relative extension paths resolve from the project root, so a todo extension configured this way keeps todo state inside each child-agent session instead of mutating the parent session.
...
Workflow code runs in a restricted sandbox. It receives direct globals instead of a context object: `agent`, `parallel`, `pipeline`, `coerce`, `mapreduce`, `verifier`, `phase`, `log`, `trace`, `args`, `cwd`, `budget`, `readText`, `readJson`, and `renderPrompt`. Workflow code cannot import modules or access ambient Node globals like `process`, but `readText` and `readJson` can read any file the pi process can read: absolute paths resolve as absolute, bare relative paths resolve from the project `cwd`, and `@workflow/...` resolves inside the workflow directory. Workflow child agents run in SDK sessions with ambient pi extensions disabled except for `workflow.childAgentExtensions`, so extension-managed tools such as todos are loaded only when explicitly configured and keep state in the child-agent session. New workflows should receive input through the default function parameter; the global `args` remains for compatibility. Agent-generated workflows must document the default workflow function with JSDoc covering purpose, input fields/defaults, phases, child agent usage, file reads, and result shape before they can be saved. Generated workflows should pass source-of-truth paths and contracts to child agents instead of pasting large source/docs contents; use `cwd`, `taskFile`, schemas, and evidence requirements, and paste only concise prior results.
...
/workflow-settings [--global] maxParallelAgents=<positive-integer>
/workflow-settings [--global] childAgentExtensions=<extension>[,<extension>...]
```
...
/workflow-review ~/.pi/agent/sessions/--home-me-project--/2026-06-26T10-00-00-review-abc12345
/workflow-settings maxParallelAgents=8
/workflow-settings --global childAgentExtensions=pi-subagents,./extensions/todo.ts
```

Named workflow commands (`/workflow <name>` and `/workflow:<name>`) start the saved workflow as a background dynamic workflow for JSON and `key=value`/`--key value` input, then return control to the chat instead of blocking the current agent turn. The TUI status line says `Waiting for 1 dynamic workflow to finish` (pluralized for multiple runs) while the workflow continues to stream progress in the workflow widget. Freeform named-workflow input is sent into the current session as a normal visible, steerable conversation: pi-workflow displays a compact prompt with `metadata.inputInstructions`, the planned `metadata.phases`, the workflow function JSDoc/signature contract, and the user's input. The agent asks if required fields are missing and calls `run_workflow` only when the input is complete. Missing required args in direct JSON/key-value calls produce an actionable message instead of starting the workflow and crashing. Every child agent launched for a workflow stores logs under `~/.pi/agent/sessions/<project-key>/<parent-run-id>/<agent-key>/`, with `metadata.json`, a compact in-flight `events.jsonl`, and the pi session JSONL. The pi session JSONL is the canonical conversation transcript; `events.jsonl` omits streamed message deltas and strips conversation payloads from message, agent-completion, and tool lifecycle events so final messages and tool results are not duplicated. Each background run also creates a temp outputs directory containing `outputs/final.json`, one `outputs/agent-*.json` file per child-agent return value, and `manifest.json`; the manifest is updated atomically from `running` to `done` or `error`. This output directory intentionally stores returned values rather than full trajectories. Token counts are never estimated; pi-workflow reports provider usage from events/session JSONL when available. Pass `--save-log` to also save the workflow-level debugging trajectory under `.pi/workflow-runs/<parent-run-id>/` with `metadata.json`, `input.json`, `workflow.js`, `events.jsonl`, `final-snapshot.json`, and the final `result.json` or `error.json`. On completion, pi-workflow posts a capped result preview plus the final result path and workflow session-log directory; that directory contains `workflow-summary.json` with planned phases, runtime phases, traces, runtime messages, fan-outs, child-agent token/tool counters, and child-agent directories whose slugs include phase, fan-out, agent id, and label for searchable follow-up analysis. `/workflow-review` now reviews those session logs, defaulting to the latest run for the project; it reports actual token spend by agent, repeated tool activity, common bash commands across agents, and concrete cost-reduction targets. If the default `workflow()` returns a string during a direct slash-command run, pi-workflow also injects that string as a hidden parent-agent handoff and triggers the current session agent; non-string results remain preview/log data only. While a workflow is running in the TUI, pi-workflow shows a collapsed live workflow row with `← for agents`, the workflow name/subtitle, completed-agent count, elapsed time, and total tokens. Press `←`, `Ctrl+\`, `F2`, or legacy `Alt-O` to open the workflow inspector. The inspector is a two-column drill-down UI: Level 1 shows `[ Phases │ agents-of-selected-phase ]` with `↑/↓` phase selection and `→`/Enter to descend; Level 2 promotes the selected phase agent list into the left column and shows the selected agent detail on the right. In detail view, `↑/↓` changes agents, `j/k` scrolls the detail body, Enter toggles prompt expansion in place, and Esc pops exactly one level (`DETAIL→PHASES→CHAT`). Agent detail uses live snapshot counters plus prompt/output previews, recent tool-call summaries, and output paths; full prompt/output values live in output files and optional logs/session transcripts rather than every live snapshot.
...
Pre-commit hooks run `lint-staged` plus `npm run docs:check:staged` through Husky after `npm install`/`npm run prepare`. The staged docs check requires JSDoc on exported declarations in public API modules and requires a staged README/docs/agent_docs/AGENTS update when staged behavior-surface files change.

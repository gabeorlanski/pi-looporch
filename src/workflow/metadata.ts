...
export function parseWorkflowSourceMetadata(
  source: string,
  workflowName: string,
  filePath = "workflow.js",
  analysis: WorkflowSourceAnalysis = analyzeWorkflowSource(source, filePath),
): WorkflowMetadata {
  const { sourceFile, metadataExpression } = analysis;

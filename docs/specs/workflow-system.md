...
- `phase(title)`
- `log(message)`
- `coerce({ schema, prompt, extensions?, tools?, ...opts })`
...
Structured agent helpers stay inside the same sandbox. `agent({ template, values }, { schema, maxAttempts?, cwd?, taskFile?, extensions?, tools? })` is the power-user path for child agents that do real work and must return a typed JSON result. `agent`, `coerce`, `mapreduce`, and `verifier` all accept `extensions?: string[]` and `tools?: string[]`; `mapreduce` applies the lists to input coercion, map workers, and reduction, while `verifier` applies them to voters and reduction. Omitted lists inherit the merged `workflow.childAgentExtensions` and `workflow.childAgentTools` settings, and a per-call list replaces its corresponding default. Missing settings mean all extensions and all tools; `[]` means none. An explicit extension list with unrestricted tools exposes the built-in tools plus every tool registered by those extensions. If an exact tool list names an extension-owned tool, the runtime loads that tool's owner even when the extension was not named, but exposes only the named tools.

The first schema-enabled launch performs the task. Later attempts up to `maxAttempts` repair the retained rejected response with `extensions: []` and `tools: []`. The runtime preflights object or boolean schemas before launch, accepts an exact JSON value or one exact fenced JSON value, and otherwise recovers only one unambiguous object or array from prose; it validates strictly, records validation failures and rejected-output artifacts as traces, and returns the parsed JSON value. `cwd` optionally launches that child agent in a different working directory; absolute paths resolve as absolute, and relative paths resolve from the workflow project's `cwd`. Treat JSON as the control surface, not the payload. Keep prompt keys and schema keys aligned, show one tiny valid output skeleton, and request one JSON value without prose or fences. Prefer compact, shallow schemas for child results: required fields, short keys, enums/booleans for status, and `additionalProperties: false`; add `maxLength` or `maxItems` only for a real downstream limit. Return only the values downstream code branches on: status, decisions, stable IDs, counts, line references, artifact paths, evidence paths, and short summaries. For large generated outputs, route content to files: prompts name exact output paths, schemas require `artifactPaths` plus concise `summary`/`status`, and final workflow results/traces/logs include paths rather than full artifact contents. Put reasoning, transcripts, reports, generated files, diffs, and bulk evidence in named files and return a compact manifest. For large lists, prefer capped summaries or JSONL/artifact files over one giant nested JSON object. If later stages need detail, first return a compact selector/manifest, then launch follow-up agents for only the selected items. Use stable IDs and lookup tables when many findings share the same long file names, commands, or criteria, so JSON entries can refer to IDs instead of repeating strings. `coerce` uses the same capability controls and strict validation flow, with `maxAttempts` counting the initial extraction plus repairs. `mapreduce` first coerces an input prompt into a bare `{ items: [...] }` shape before map fan-out and reduction; select and cap items before fan-out. `verifier` validates criteria objects with `name`, `description`, `guidelines`, `reasoning`, and optional `voters`, then runs criterion voter agents before a reduction agent; prefer one voter unless deeper review is explicitly required. `log(message)` records live user-facing milestones before slow launches, after important decisions/results, and at handoffs; `trace(label, value?)` records structured workflow-local debug state for selected inputs, counts, paths, artifact paths, handoff state, and compact child-agent result summaries.

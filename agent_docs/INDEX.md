...
- `run_workflow` returns a run id; `abort_workflow({ runId })` requests cooperative cancellation of an exact run in the same live Pi session and immediately returns artifact locations. It is idempotent for aborting or terminal runs, and rejects unknown or other-session IDs. `resume_workflow` resumes failed or aborted runs only in the same live Pi session. Replay uses the original input and current workflow source, reuses the unchanged successful `agent`/`LLM` prefix, and runs normally from the first changed or incomplete call.

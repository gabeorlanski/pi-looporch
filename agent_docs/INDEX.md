...
- Workflow completion messages should surface the final result or report, keep output/session-log paths visible, and for command-launched workflows inject a visible non-human follow-up handoff that triggers the current agent to review and summarize the result.
...
- Workflow completion handoffs should be visible to the user and clearly marked as automated. Do not hide result-review prompts; include a bounded result preview plus paths so the agent can read more if needed.

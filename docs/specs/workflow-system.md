...
The TUI progress display keeps a compact normalized input preview visible, keeps phase history visible, and shows active/error child agents with status, model/reasoning, provider input/output tokens, tool calls, and step counts. Completed agents collapse into a count. The default progress view does not show the runtime log or child-agent messages. Detailed milestones, traces, and child tool transitions stay available in persisted session events and transcripts.

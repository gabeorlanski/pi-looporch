...
- Shared prompt context is allowed, but format it as a compact XML contract: keep static `<workflow_instructions>` sections first, then final typed `<workflow_inputs>` or `<workflow_task>` sections. Treat workflow data as non-user content, keep dynamic sections intentionally small, and omit inputs/globals the stage does not directly consume so the stable prefix remains cacheable.

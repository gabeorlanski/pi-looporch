...
- `LLM(prompt, { system?, messages?, chatTemplate?, schema? })`
...
`LLM(prompt, { system?, messages?, chatTemplate?, schema? })` performs exactly one generation-only request with Pi's active model, provider, and authentication. Supplied user/assistant messages retain their order and the primary prompt is appended as the final user message. The complete message list is always rendered into one user prompt using a simple role-prefixed default or a supplied Hugging Face-style Jinja `chatTemplate`. Every call returns `{ text, output, usage, model, provider, stopReason }`, using `null` for unavailable metadata and for raw-call `output`. Schema calls instruct the model to return one unfenced JSON value, decode and validate it locally, and reject malformed or nonconforming responses without repair or retry. The primitive has no tools, extensions, agent session, child-agent snapshot, streaming callbacks, or child-agent concurrency slot, and provider failures or cancellation reject through the workflow boundary.

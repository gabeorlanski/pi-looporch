...
Project workflows should live in the pi project config directory, normally `.pi/workflows/<workflow-name>/`.

The only required file is:

```text
<project-config-dir>/workflows/<workflow-name>/workflow.js
```
...
Discovery is a startup and autocomplete boundary, so invalid workflow definitions must not crash pi. Project-local workflow discovery runs only after pi trusts the project; untrusted projects expose no project workflows or project workflow settings. If a workflow file under the project config directory, normally `.pi/workflows/<name>/workflow.js`, cannot be parsed for metadata or violates sandbox source rules such as the import ban, discovery skips that workflow until the file is fixed and logs the skipped path.
...
Generated new workflows are saved directly from complete draft directories. `propose_workflow` validates a draft under the project config directory, normally `.pi/workflow-drafts/<name>/workflow.js`, plus any workflow-local resources, then copies the directory to the workflows directory, normally `.pi/workflows/<name>/`, in one call. Saved workflows run through `run_workflow` or a named workflow command.
...
Workflow runtime settings are configured with `/workflow-settings`. With no args, it posts a readable settings message with active values, storage locations, trust state, and direct edit commands. With args, `/workflow-settings maxParallelAgents=<n>` or `/workflow-settings childAgentExtensions=<extension>[,<extension>...]` writes the project settings value under the project config directory, normally `.pi/settings.json`, directly; prefix `--global` to write global `~/.pi/agent/settings.json` instead. Relative child-agent extension paths resolve from the workflow project root. Configured `workflowDirs` are project-scope only; global settings do not add workflow discovery roots.

## Runtime UI

Normal TUI execution should stay lean. Named workflow commands should execute saved workflows directly rather than routing through a session-agent `run_workflow` tool call. Workflows started by named commands and workflows started by the current-session agent through the `run_workflow` tool should both show the same compact below-editor widget with workflow name, subtitle, agent counts, elapsed time, and token totals. Active runs are registered as transient per-run files under the project config directory, normally `.pi/workflow-runs/active/<run-id>.json`, and deleted when the run settles; each active record stores the parent Pi session id, and reattachment filters by that session so a different session in the same project directory does not adopt the run. The latest live snapshot is persisted as `snapshot.json` in the workflow output directory so the widget and `/view-workflow` can reattach after extension reloads. Snapshot agents carry paths to prompt, activity, and output artifacts rather than embedding those payloads. Pressing ↓ on an empty prompt selects the widget, Enter opens a full-screen phase/agent inspector, and Esc/↑ returns to the prompt. `/view-workflow` opens that same inspector directly for the active running workflow and warns when none is running. The inspector shows full details, loads the three most recent exact tool uses and exact output from artifacts, and keeps exact prompts collapsed until Enter is pressed. Completion messages still point to output files and session logs rather than embedding workflow result content in the parent conversation. Deeper cost and transcript inspection happens through the completion paths, `/workflow-review`, and child session logs.

New-workflow saving (`propose_workflow`) is direct. Agents pass a project-local draft directory containing `workflow.js` and any workflow-local prompts/assets; the `draftDir` argument points to that directory, not to the `workflow.js` file. The tool validates the draft and copies it to the project workflows directory, normally `.pi/workflows/<name>/`.
